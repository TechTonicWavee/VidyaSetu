...
export function evaluateCertificate(cert, studentContext = {}) {
  ...
  if (recipientName && recipientName.trim().length > 0 && studentName && studentName.trim().length > 0) {
    ...
    if (hasNameMatch) {
      ...
      // Recipient name explicitly provided but does NOT match logged in student -> Fraud/Identity spoofing!
      identityMismatch = true
      verifiabilityScore = 1 // Heavy penalty: override verifiability score to 1
    }
  }

  const totalScore = issuerScore + rigorScore + relevanceScore + verifiabilityScore // Max 20
