...
import type { DeanStudent } from '../mock-data'
...
export interface EnrichedStudent extends DeanStudent {
  category: CategoryId
...
// ─── Category metadata ────────────────────────────────────────────────────────
export const CATEGORY_META: Record<string, { issue: string; action: string; icon: string }> = {
...
export function classifyStudent(s: DeanStudent): CategoryId {
...
type ScoreKey = 'academicScore' | 'dsaScore' | 'devScore' | 'softSkillScore' | 'attendance' | 'aptitudeScore' | 'extracurricularScore'
...
export function enrichStudents(students: DeanStudent[]): EnrichedStudent[] {
...
export interface CategoryDistributionEntry extends CategoryMeta {
  count: number
  pct: number
}

// ─── Distribution count ───────────────────────────────────────────────────────
export function getCategoryDistribution(enriched: EnrichedStudent[]): CategoryDistributionEntry[] {
  const dist: Record<string, number> = {}
  Object.keys(CATEGORIES).forEach(k => { dist[k.toLowerCase()] = 0 })
  enriched.forEach(s => { dist[s.category] = (dist[s.category] || 0) + 1 })
  return Object.entries(CATEGORIES).map(([, meta]) => ({
    ...meta,
    id: meta.id,
...
export function generateInsights(enriched: EnrichedStudent[], dist: CategoryDistributionEntry[]): Insight[] {
...
// ─── Top 5 per category ───────────────────────────────────────────────────────
export function getTopPerCategory(enriched: EnrichedStudent[], categoryId: CategoryId, n = 5): EnrichedStudent[] {
  return enriched
...
// ─── Worst cluster ────────────────────────────────────────────────────────────
export function getWorstCluster(enriched: EnrichedStudent[], n = 5): EnrichedStudent[] {
