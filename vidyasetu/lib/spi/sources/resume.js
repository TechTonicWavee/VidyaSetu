...
// Section keys for quality evaluation
const SECTION_KEYS = [
...
function getProfessionalPresence(resumeParsed, targetPresence) {
  ...
  const fullText = lc(raw + ' ' + summary)

  let actual = 0
  const found = []

  if (/[\w.+-]+@[\w-]+\.[\w.]+/.test(raw))   { actual++; found.push('email') }
  if (/[\+]?[\d\s\-().]{7,}/.test(raw))       { actual++; found.push('phone') }
  if (/github\.com/i.test(raw))               { actual++; found.push('github') }
  if (/linkedin\.com/i.test(raw))             { actual++; found.push('linkedin') }

  // Portfolio: any personal website not github/linkedin
  if (
    fullText.includes('portfolio') ||
    fullText.includes('website')   ||
    /(?:https?:\/\/)?(?:www\.)?[\w-]+\.(?:com|io|me|net|org|dev)(?!.*(?:github|linkedin))/.test(raw)
  ) {
    actual++
 ...
 *
 * @param {object} params
 * @param {object|null} params.resumeParsed — parsed resume JSON from DB
 * @param {number}      params.year         — student academic year (1–4)
 * @returns {object} score, semester, targets, breakdown, metadata
 */
export function calcResumeScore({ resumeParsed, year } = {}) {
