...
// ─── Animated Counter ─────────────────────────────────────────────────────────
function CountUp({ to, duration = 800, suffix = '' }: { to: number; duration?: number; suffix?: string }) {
...
// ─── Custom Pie Tooltip ───────────────────────────────────────────────────────
const PieTooltip = ({ active, payload }: { active?: boolean; payload?: { payload: { label: string; color: string; count: number; pct: number } }[] }) => {
...
export default function StudentIntelligence() {
  ...
  const [sortKey,        setSortKey]        = useState<keyof EnrichedStudent>('composite')
  ...
  const toggleSort = (key: keyof EnrichedStudent) => {
  ...
  const categoryRadar = useMemo(() => {
    ...
    const avg = (field: keyof EnrichedStudent) => +(cats.reduce((s, x) => s + (x[field] as number), 0) / cats.length).toFixed(1)
    return [
            ...
            <ResponsiveContainer minWidth={1} minHeight={1} width="100%" height={240}>
              <BarChart
                data={distribution.filter(d => d.count > 0).map(d => {
                  ...
                  const avg = (f: keyof EnrichedStudent) => cats.length ? Math.round(cats.reduce((s, x) => s + (x[f] as number), 0) / cats.length) : 0
                  return {
