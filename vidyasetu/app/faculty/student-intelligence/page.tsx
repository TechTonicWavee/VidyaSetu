...
type RiskyStudent = SectionStudent & { risk: RiskInfo }
...
// ─── Risk classifier ─────────────────────────────────────────────────────────
function classifyRisk(s: SectionStudent): RiskInfo {
...
function attPct(s: SectionStudent) {
...
// ─── Animated counter ────────────────────────────────────────────────────────
function CountUp({ to, duration = 700, suffix = '' }: { to: number; duration?: number; suffix?: string }) {
...
// ─── Attendance cell ─────────────────────────────────────────────────────────
function AttWeek({ week }: { week: { week: string; present: number; total: number } }) {
...
function StudentDrawer({ student, risk, onClose, notes, onSaveNote, onFlag }: StudentDrawerProps) {
...
const BenchTooltip = ({ active, payload, label }: { active?: boolean; payload?: { fill: string; name: string; value: number }[]; label?: string }) => {
...
export default function FacultyStudentIntelligence() {
  ...
  const [students, setStudents] = useState<RiskyStudent[]>(() =>
    SECTION_STUDENTS.map(s => ({ ...s, risk: classifyRisk(s) }))
  )
  const [notes, setNotes] = useState<Record<number, string>>({})   // { studentId: noteText }

  // ── Filter state ──────────────────────────────────────────────────────────
  const [search,      setSearch]      = useState('')
  const [filterRisk,  setFilterRisk]  = useState('ALL')
  const [filterAssign,setFilterAssign]= useState('ALL')  // ALL | low | high
  const [sortKey,     setSortKey]     = useState<keyof RiskyStudent>('name')
  const [sortAsc,     setSortAsc]     = useState(true)
  const [showFlagged, setShowFlagged] = useState(false)

  // ── Drawer ────────────────────────────────────────────────────────────────
  const [drawerStudent, setDrawerStudent] = useState<RiskyStudent | null>(null)
  ...
  const handleSaveNote = (id: number, text: string) => {
    setNotes(prev => ({ ...prev, [id]: text }))
    setToastMsg('Note saved')
    setTimeout(() => setToastMsg(''), 2500)
    // update drawer student if open
    if (drawerStudent?.id === id) setDrawerStudent(s => (s ? { ...s } : s))
  }
  ...
  const riskDist = useMemo(() => {
    const map: Record<string, number> = { critical: 0, at_risk: 0, watch: 0, average: 0, strong: 0 }
  ...
  const toggleSort = (key: keyof RiskyStudent) => {
    if (sortKey === key) setSortAsc(v => !v)
    else { setSortKey(key); setSortAsc(key === 'name') }
  }

  const subjectScore = (s: RiskyStudent) => activeSubject === 'CS201' ? s.dbmsScore : s.osScore
    ...
    <div className="flex h-screen bg-[#F3F4F6] overflow-hidden font-sans">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white border-b border-gray-100 px-6 py-3 flex items-center gap-4 flex-shrink-0 shadow-sm z-10">
          ...
          <button onClick={() => router.push('/faculty/alerts')} className="relative p-2 rounded-lg hover:bg-gray-100 transition text-gray-500">
            <Bell size={19} />
              ...
              {insightOpen && (
                <div className="space-y-2.5 overflow-y-auto max-h-64">
                  {insights.map((ins, i) => {
                    ...
                    } as Record<string, { dot: string; bg: string; border: string; text: string }>)[ins.type]
                    return (
            ...
            <div className="xl:col-span-2 bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
              ...
              <ResponsiveContainer minWidth={1} minHeight={1} width="100%" height={180}>
                <ReBarChart data={benchData} barSize={14}>
                  ...
                  <YAxis domain={[0, 100]} tick={{ fontSize: 10 }} />
                  <Tooltip content={BenchTooltip as any} />
                  <Legend iconSize={8} wrapperStyle={{ fontSize: 10 }} />
                  ...
                  <tr className="bg-gray-50 border-b border-gray-100">
                    {[
                      ...
                      <th key={ci} className={`px-4 py-3 text-left text-[10px] font-bold text-gray-400 uppercase tracking-wider ${col.w}`}>
                        {col.key ? (
                          <button onClick={() => toggleSort(col.key as keyof RiskyStudent)} className="flex items-center gap-1 hover:text-gray-700 transition">
                            {col.label}
