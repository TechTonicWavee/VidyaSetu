...
type StepPosition = 'top-right' | 'bottom-left' | 'bottom-center' | 'left-center'
...
const DEMO_STEPS: DemoStep[] = [
  ...
  {
    step: 5,
    route: '/parent', // The parent view route
    position: 'top-right',
    ...
    <DemoContext.Provider value={{ isDemoActive, startDemo, stopDemo, currentStepIndex, nextStep, prevStep }}>
      ...
      {isDemoActive && !showCompletion && (
        <DemoTourCard 
          ...
          onNext={nextStep}
          onPrev={prevStep}
          onSkip={() => { stopDemo(); router.push('/student') }}
...
function DemoTourCard({ stepInfo, totalSteps, onNext, onPrev, onSkip, isFirst, isLast }: DemoTourCardProps) {
  // Map position string to CSS classes
  const posClasses: Record<StepPosition, string> = {
...
function DemoCompletionOverlay({ onExplore, onBackToLogin }: { onExplore: () => void; onBackToLogin: () => void }) {
  return (
