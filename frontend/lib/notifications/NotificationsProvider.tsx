...
export function NotificationsProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      // Transient failure (e.g. a DB reconnect) shouldn't crash the app — the next
      // refresh() (poll, reconnect, or manual retry) will catch it back up.
  ...
  useEffect(() => {
    ...
    const onNew = (notification: Notification) => {
      ...
      addToast(notification.title, 'info', notification.body ?? '', 6000);
    };
  ...
  return (
    <NotificationsContext.Provider
      value={{ unreadCount, recent, loading, refresh, markRead, markAllRead }}
    >
      {children}
    </NotificationsContext.Provider>
  );
