...
function genAttendanceTimeline(rng: () => number, baseAtt: number) {
  const weeks = []
  for (let w = 0; w < 8; w++) {
    // 3 classes per week
...
function genSubjectScores(rng: () => number, academicBase: number) {
  return {
...
export interface SectionStudent {
  ...
  parentPhone: string
  parentEmail: string
  attendanceTimeline: { week: string; present: number; total: number }[]
...
export function generateSectionStudents(count = 80, seed = 777): SectionStudent[] {
