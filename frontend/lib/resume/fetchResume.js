 ...
 */
export async function fetchResume(resumeUrl) {
  // ── Validate input ────────────────────────────────────────────────────────
  ...
  try {
    ...
    throw new Error(
      `[fetchResume] Network request failed for URL "${url}": ${networkError.message}`
    )
  }
  ...
  try {
    ...
    throw new Error(
      `[fetchResume] Failed to read response body from "${url}": ${readError.message}`
    )
  }
