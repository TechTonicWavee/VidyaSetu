...
async function refreshAccessToken(): Promise<boolean> {
  if (!refreshPromise) {
    refreshPromise = (async () => {
      try {
        const res = await fetch(resolveApiUrl('/api/auth/refresh'), { method: 'POST', credentials: 'include' });
...
// Base URL for the API. Empty string => same-origin Next.js API routes (default).
// Set NEXT_PUBLIC_API_BASE_URL (e.g. http://localhost:4000) to route through the
// standalone Express backend without touching any page/component.
