 ...
 */
export default function CollapsibleSidebar({ navLinks, userInfo, theme }: CollapsibleSidebarProps) {
  ...
  return (
    ...
    >
      ...
      >
        <button
          onClick={toggleTheme}
          title={!expanded ? (appTheme === 'dark' ? 'Switch to light mode' : 'Switch to dark mode') : undefined}
          style={{
            display: 'flex',
            alignItems: 'center',
            gap: 12,
        ...
        >
          {appTheme === 'dark' ? (
            <Sun size={19} style={{ flexShrink: 0, color: t.iconColor }} />
          ) : (
            <Moon size={19} style={{ flexShrink: 0, color: t.iconColor }} />
          )}
          <span
            style={{
              fontSize: 13,
