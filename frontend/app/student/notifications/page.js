...
export default function NotificationsPage() {
  ...
  const [allNotifs, setAllNotifs] = useState(notificationsData)

  useEffect(() => {
    const existingStr = localStorage.getItem('custom_notifs')
    if (existingStr) {
      try {
        const customNotifs = JSON.parse(existingStr)
        setAllNotifs(prev => {
          // avoid duplicates if effect runs twice in strict mode
          const newMap = new Map(customNotifs.map(n => [n.id, n]))
          const filteredPrev = prev.filter(p => !newMap.has(p.id))
          return [...customNotifs, ...filteredPrev]
        })
      } catch (e) { }
    }
  }, [])

  const filteredNotifs = allNotifs.filter(n => activeFilter === 'All' || n.category === activeFilter)
                      ...
                      <div key={n.id} className="bg-blue-50/50 border border-blue-100 rounded-2xl p-5 shadow-sm hover:shadow transition relative overflow-hidden">
                        ...
                        <div className="flex gap-4">
                          <div className={`w-10 h-10 rounded-full bg-${n.color}-100 flex items-center justify-center flex-shrink-0`}>
                            {n.iconName === 'Users' ? <Users size={18} className={`text-${n.color}-600`} /> : <n.icon size={18} className={`text-${n.color}-600`} />}
                          </div>
                          <div className="flex-1">
