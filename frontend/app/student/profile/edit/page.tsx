...
function CollapsibleSection({ title, icon: Icon, children, isOpen, onToggle, completionPercent, badge }: CollapsibleSectionProps) {
  const panelId = `section-${title.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')}`
  return (
    <div className="bg-surface rounded-lg border border-line overflow-hidden shadow-sm hover:shadow-md transition-shadow">
      <button
        onClick={onToggle}
        aria-expanded={isOpen}
        aria-controls={panelId}
        className="w-full p-5 flex items-center justify-between bg-surface hover:bg-surface-2 transition-colors"
      >
            ...
            {completionPercent !== undefined && (
              <div className="text-xs text-muted mt-0.5">
                <div className="w-24 h-1.5 bg-surface-3 rounded-full overflow-hidden mt-1">
                  <div className="h-full bg-success transition-all" style={{ width: `${completionPercent}%` }} />
                </div>
              </div>
            )}
          </div>
        </div>
        <ChevronDown size={20} className={`text-muted transition-transform ${isOpen ? 'rotate-180' : ''}`} aria-hidden />
      </button>
      {isOpen && (
        <div id={panelId} role="region" aria-label={title} className="border-t border-line p-5 bg-surface-2">
          {children}
        </div>
      )}
    </div>
...
export default function ProfileEditPage() {
      ...
      {uploadingResume ? (
        ...
        <div className="bg-surface border border-line rounded-lg p-5">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center flex-shrink-0 text-brand">
                <FileText size={24} />
              </div>
              <div>
                <p className="text-xs text-muted font-medium">Current Resume</p>
                <h4 className="font-semibold text-content text-base">resume.pdf</h4>
              </div>
            </div>
            <div className="flex items-center gap-2">
          ...
          {(resumeAnalyzedAt || resumeParsed || resumeScore != null) && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-b border-line py-4 mb-4">
              {resumeAnalyzedAt && (
                <div>
                  <p className="text-xs text-muted font-medium">Last Updated</p>
              ...
              {resumeParsed && (
                <div>
                  <p className="text-xs text-muted font-medium">Resume Parsed</p>
              ...
              {resumeScore != null && (
                <div>
                  <p className="text-xs text-muted font-medium">Resume Score</p>
          ...
          <div className="flex flex-wrap gap-2">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-4 py-2 border border-line text-content rounded-lg text-sm font-medium hover:bg-surface-2 transition-colors flex items-center gap-1.5"
            >
        ...
        <div className="bg-surface border border-line rounded-lg p-6">
          <div className="flex flex-col items-center justify-center py-6 text-center">
            <div className="w-12 h-12 rounded-xl bg-surface-2 flex items-center justify-center mb-3 text-muted">
              <FileText size={24} />
            </div>
            <h4 className="font-semibold text-content text-base mb-1">No resume uploaded</h4>
            <p className="text-sm text-muted mb-4">Upload your resume (PDF only, max 10MB) to include it in your SPI calculation.</p>
            <button
              onClick={() => document.getElementById('resume-file-input')?.click()}
              className="px-5 py-2.5 bg-brand text-white rounded-lg text-sm font-semibold hover:bg-brand-hover transition-colors flex items-center gap-2 shadow-sm"
            >
  ...
  const basicInfoJSX = (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="text-sm font-medium text-content mb-1 flex items-center gap-1.5">
            Full Name <Lock size={12} className="text-muted" />
          </label>
          <input
            type="text"
            value={basicInfo.name || ''}
            readOnly
            className="w-full px-4 py-2.5 rounded-lg border border-line bg-surface-2 text-sm text-muted cursor-not-allowed"
          />
          <p className="text-xs text-muted mt-1">Set by your institution. Contact admin to update.</p>
        </div>
        <div>
      ...
      <div>
        <label className="text-sm font-medium text-content mb-1 flex items-center gap-1.5">
          Email <Lock size={12} className="text-muted" />
        </label>
        <input
          type="email"
          ...
          className="w-full px-4 py-2.5 rounded-lg border border-line bg-surface-2 text-sm text-muted cursor-not-allowed"
        />
        <p className="text-xs text-muted mt-1">Your registered email. Contact admin to update.</p>
      </div>
    </div>
  )
        ...
        {codingPlatformFields.map(field => (
          <div key={field.key}>
            ...
            <div className="flex rounded-lg shadow-sm">
              <span className="inline-flex items-center px-3 rounded-l-lg border border-r-0 border-line bg-surface-2 text-muted text-xs select-none">
                {field.hint}
              </span>
              <input
                type="text"
  ...
  const projectsJSX = (
    <div className="space-y-4">
      ...
      {projects.map(project => (
        <div key={project.id} className="bg-surface border border-line rounded-lg p-4">
          ...
          {project.description && <p className="text-sm text-content-2 mb-2">{project.description}</p>}
          {project.techStack && project.techStack.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-2">
              {project.techStack.map((tech, i) => (
                <span key={i} className="text-xs bg-blue-50 text-brand px-2 py-1 rounded">{tech}</span>
              ))}
            </div>
          )}
          <div className="flex gap-2 text-xs">
            {project.github && <a href={project.github} target="_blank" rel="noreferrer" className="text-brand hover:underline flex items-center gap-1"><Github size={14} /> Code</a>}
            {project.liveDemo && <a href={project.liveDemo} target="_blank" rel="noreferrer" className="text-brand hover:underline flex items-center gap-1"><ExternalLink size={14} /> Demo</a>}
          </div>
        </div>
      ))}

      <div className="bg-surface border border-line rounded-lg p-4">
        <h4 className="font-semibold text-content mb-3">Add New Project</h4>
        <input
          type="text"
          value={newProject.title}
          onChange={e => setNewProject({ ...newProject, title: e.target.value })}
          placeholder="Project title *"
          className="w-full px-3 py-2 mb-2 rounded border border-line focus:outline-none focus:ring-2 focus:ring-brand/40 text-sm text-content"
        />
        <textarea
          value={newProject.description}
          onChange={e => setNewProject({ ...newProject, description: e.target.value })}
          placeholder="Project description"
          className="w-full px-3 py-2 mb-2 rounded border border-line focus:outline-none focus:ring-2 focus:ring-brand/40 text-sm resize-none text-content"
          rows={2}
        />
        <input
          type="text"
        ...
        <div className="grid grid-cols-2 gap-2 mb-2">
          <select
            ...
            className="px-3 py-2 rounded border border-line focus:outline-none focus:ring-2 focus:ring-brand/40 text-sm text-content"
          >
        ...
        <div className="grid grid-cols-2 gap-2 mb-2">
          <input
            ...
            className="px-3 py-2 rounded border border-line focus:outline-none focus:ring-2 focus:ring-brand/40 text-sm text-content w-full"
          />
          <input
            type="url"
        ...
        <button
          onClick={addProject}
          className="w-full px-3 py-2 bg-brand text-white rounded text-sm font-medium hover:bg-brand-hover transition-colors flex items-center justify-center gap-2"
        >
  ...
  const certificationsJSX = (
    <div className="space-y-4">
      ...
      {certifications.map(cert => {
        ...
        return (
          <div key={cert.id} className="bg-surface border border-line rounded-lg p-4 shadow-sm">
                ...
                <div className="flex items-center flex-wrap gap-2 mb-1">
                  ...
                  {cert.tier && (
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                      ...
                      'bg-surface-3 text-content border border-gray-300'
                    }`}>
            ...
            <div className="my-2 p-2.5 bg-surface-2 rounded border border-line text-xs space-y-1">
              <div className="flex items-center justify-between">
                <span className="font-medium text-content">Credential ID:</span>
                <span className={cert.credentialId ? 'font-mono text-gray-800 bg-white px-1.5 py-0.5 rounded border border-gray-200' : 'text-muted italic'}>
              ...
              {cert.verificationUrl ? (
                <div className="flex items-center justify-between pt-1">
                  <span className="font-medium text-content">Verification URL:</span>
                  <a
                ...
                <div className="flex items-center justify-between pt-1">
                  <span className="font-medium text-content">Verification URL:</span>
                  <span className="text-muted italic">None provided (Add Credly/AWS link below)</span>
                </div>
              )}
            </div>
      ...
      <div className="bg-surface border border-line rounded-lg p-4">
        <h4 className="font-semibold text-content mb-3">Add New Certification</h4>
        <input
          type="text"
          value={newCert.name}
  ...
  const internshipsJSX = (
    <div className="space-y-4">
      ...
      {internships.map(item => {
        ...
        return (
          <div key={item.id} className="bg-surface border border-line rounded-lg p-4 shadow-sm">
            <div className="flex justify-between items-start mb-2">
              <div>
                <div className="flex items-center flex-wrap gap-2 mb-1">
                  <h4 className="font-semibold text-content">{item.company}</h4>
            ...
            {item.description && <p className="text-sm text-content-2 mb-2">{item.description}</p>}
      ...
      <div className="bg-surface border border-line rounded-lg p-4">
        <h4 className="font-semibold text-content mb-3">Add New Internship</h4>
        <div className="grid grid-cols-2 gap-2 mb-2">
          <input
            type="text"
        ...
        <div className="grid grid-cols-2 gap-2 mb-2">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded border border-line focus:outline-none focus:ring-2 focus:ring-brand/40 text-sm text-content"
            />
          </div>
          <div>
            <label className="text-xs font-medium text-muted block mb-1">End Date (leave blank if current)</label>
            <input
              type="date"
        ...
        <div className="grid grid-cols-2 gap-2 mb-2 items-center bg-surface-2 p-2.5 rounded border border-line">
          <div className="flex items-center gap-2">
            <input
              type="checkbox"
              id="isPaidCheck"
  ...
  const hackathonsJSX = (
    <div className="space-y-4">
      ...
      {hackathons.map(hack => (
        <div key={hack.id} className="bg-surface border border-line rounded-lg p-4">
          <div className="flex justify-between items-start mb-2">
            <div>
          ...
          <div className="text-sm text-content-2 space-y-1">
      ...
      <div className="bg-surface border border-line rounded-lg p-4">
        <h4 className="font-semibold text-content mb-3">Add Hackathon / Competition</h4>
        <input
          type="text"
          value={newHack.name}
  ...
  const extracurricularsJSX = (
    <div className="space-y-4">
      ...
      {extracurriculars.map(extra => (
        <div key={extra.id} className="bg-surface border border-line rounded-lg p-4">
          <div className="flex justify-between items-start mb-2">
            <div>
      ...
      <div className="bg-surface border border-line rounded-lg p-4">
        <h4 className="font-semibold text-content mb-3">Add Activity</h4>
        <input
          type="text"
          value={newExtra.name}
        ...
        {/* Profile Completion Bar */}
          ...
          <div className="space-y-4 max-w-4xl">
            ...
            <button
              ...
              disabled={saving}
              className="w-full py-3 bg-brand text-white rounded-xl font-semibold text-sm hover:bg-brand-hover disabled:opacity-60 transition-all flex items-center justify-center gap-2 shadow-md"
            >
              {saving ? (
