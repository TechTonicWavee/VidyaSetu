...
import { useEffect, useState, useRef, type DragEvent, type MouseEvent as ReactMouseEvent, type KeyboardEvent, type ChangeEvent } from 'react'
...
interface FormDataShape {
  ...
  extracurriculars: ExtraEntry[]
  internships: InternEntry[]
}

type FormFieldChange = <K extends keyof FormDataShape>(key: K, value: FormDataShape[K]) => void
...
const emptyCert = (): CertEntry => ({ name: '', platform: 'Coursera', date: '', skills: [], certificateUrl: null, certificateFileName: null })
const emptyExtra = (): ExtraEntry => ({ society: '', role: '', year: '2024-25', achievement: '' })
const emptyIntern = (): InternEntry => ({ company: '', role: '', startDate: '', endDate: '', techStack: [], description: '', offerLetterUrl: null, offerLetterFileName: null })
...
function ProgressBar({ currentStep, completedSteps, onStepClick }: { currentStep: number; completedSteps: number[]; onStepClick: (step: number) => void }) {
  return (
...
function FileUploadZone({
  ...
  const handleRetry = (e: ReactMouseEvent) => {
    e.stopPropagation()
    if (selectedFile) {
...
function Step1({ data, onChange, onNext, errors }: StepBaseProps & { onNext: () => void; errors: Record<string, boolean> }) {
  return (
    <div>
      ...
      <div className="space-y-4">
        {PLATFORM_FIELDS.map((field) => (
          <div key={field.key}>
            ...
            <input
              type="text"
              value={data.coding_profiles[field.key as keyof CodingProfiles]}
...
function Step2({ data, onChange, onNext, onPrev, onSkip, universityId }: StepBaseProps & StepNavProps) {
  return (
    <div>
...
function Step3({ data, onChange, onNext, onPrev, onSkip, universityId }: StepBaseProps & StepNavProps) {
  const updateCert = <K extends keyof CertEntry>(idx: number, field: K, value: CertEntry[K]) => {
  ...
  const addSkill = (idx: number, skill: string) => {
    ...
    if (cert && !cert.skills.includes(skill)) updateCert(idx, 'skills', [...cert.skills, skill])
  }
...
function Step4({ data, onChange, onPrev, onSubmit, submitting, universityId }: StepBaseProps & { onPrev: () => void; onSubmit: () => void; submitting: boolean; universityId?: string }) {
  const updateExtra = <K extends keyof ExtraEntry>(idx: number, field: K, value: ExtraEntry[K]) => {
    const updated = data.extracurriculars.map((e, i) => i === idx ? { ...e, [field]: value } : e)
    onChange('extracurriculars', updated)
  }

  const updateIntern = <K extends keyof InternEntry>(idx: number, field: K, value: InternEntry[K]) => {
    const updated = data.internships.map((e, i) => i === idx ? { ...e, [field]: value } : e)
    onChange('internships', updated)
  }

  const addTechStack = (idx: number, tag: string) => {
    const intern = data.internships[idx]
    if (intern && !intern.techStack.includes(tag)) updateIntern(idx, 'techStack', [...intern.techStack, tag])
  }

  const removeTechStack = (idx: number, tag: string) => {
...
export default function FormPage() {
  ...
  const [formData, setFormData] = useState<FormDataShape>(defaultFormData())
  ...
  useEffect(() => {
    ...
    try {
      const parsed: StudentSession = JSON.parse(raw)
  ...
  const saveProgress = (newStep: number, newCompleted: number[], newData: FormDataShape) => {
  ...
  const updateField: FormFieldChange = (key, value) => {
