      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-5xl">
        ...
        <div className="bg-[#0f1f30] border border-gray-700/50 rounded-2xl p-8 flex flex-col transition-transform hover:scale-[1.01]">
          ...
          <div className="flex flex-wrap gap-3">
            <RoleButton href="/student" icon={<User size={16} />} label="Student" />
            <RoleButton href="/faculty" icon={<BookOpen size={16} />} label="Faculty" />
            <RoleButton href="/dean" icon={<Shield size={16} />} label="Dean" />
            <RoleButton href="/parent" icon={<Users size={16} />} label="Parent" />
            <RoleButton href="/admin" icon={<Settings size={16} />} label="Admin" />
          </div>
        </div>
      </div>
...
function RoleButton({ href, icon, label }: { href: string; icon: ReactNode; label: string }) {
  return (
    <Link 
      href={href}
