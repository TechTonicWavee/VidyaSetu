...
const initialFaculty = [
  ...
  { id: 2, name: 'Prof. Priya Kapoor', empId: 'EMP002', dept: 'CSE', subjects: ['DBMS', 'OS'], students: 243, status: 'Active', login: 'Today 8:52 AM', avatar: 'PK', color: 'bg-brand' },
  ...
  { id: 4, name: 'Prof. Meena Rao', empId: 'EMP004', dept: 'IT', subjects: ['Web Tech', 'Soft Engg'], students: 210, status: 'Active', login: 'Today 10:15 AM', avatar: 'MR', color: 'bg-brand' },
...
export default function ConfigurationPanel() {
  ...
  const [editingFaculty, setEditingFaculty] = useState<(typeof initialFaculty)[number] | null>(null)
  const [deactivateModal, setDeactivateModal] = useState<{ open: boolean; id: number | null; name: string; action?: string }>({ open: false, id: null, name: '' })
  ...
  const handleEditFaculty = (fac: (typeof initialFaculty)[number] | null) => {
  ...
  const getSpiColor = (spi: number) => {
    ...
    if (spi < 75) return 'text-brand font-bold'
  ...
  return (
    <div className="space-y-8 pb-8 animate-fade-in">
      <PageHeader 
        ...
        description="Manage faculty accounts, student records, branches, sections and system-wide settings"
        actions={
          <div className="flex items-center gap-2 px-3 py-1.5 bg-success-soft border border-success-soft rounded-lg text-success text-sm font-bold shadow-card">
      ...
      {/* Quick Stats Strip */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
              <div className="bg-surface rounded-2xl shadow-card border border-line p-5 flex items-start gap-4">
                <div className="w-10 h-10 rounded-full bg-brand-soft flex items-center justify-center text-brand shrink-0"><UserCheck size={20} /></div>
                <div>
                  <p className="text-3xl font-bold text-brand leading-none mb-1">48</p>
                  <p className="text-xs font-bold text-navy">Total Faculty</p>
                  <p className="text-[10px] text-muted mt-1">18 CSE · 16 IT · 14 ECE</p>
                </div>
              </div>
              <div className="bg-surface rounded-2xl shadow-card border border-line p-5 flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-muted mt-1">Across 3 branches</p>
                </div>
              </div>
              <div className="bg-surface rounded-2xl shadow-card border border-line p-5 flex items-start gap-4">
                <div className="w-10 h-10 rounded-full bg-brand-soft flex items-center justify-center text-brand shrink-0"><Activity size={20} /></div>
                <div>
                  <p className="text-3xl font-bold text-brand leading-none mb-1">342</p>
                  <p className="text-xs font-bold text-navy">Active Sessions</p>
                  <p className="text-[10px] text-muted mt-1">Right now on platform</p>
                </div>
              </div>
              <div className="bg-surface rounded-2xl shadow-card border border-line p-5 flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-muted mt-1">Require admin attention</p>
                </div>
              </div>
              <div className="bg-surface rounded-2xl shadow-card border border-line p-5 flex items-start gap-4">
                <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-muted shrink-0"><Database size={20} /></div>
                <div>
                  ...
                  <p className="text-[10px] text-muted mt-1">of 50 GB allocated</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-surface rounded-xl shadow-card border border-line overflow-hidden">
              ...
              {activeTab === 'Faculty Management' && (
                <div className="animate-fade-in">
                  <div className="p-4 border-b border-line bg-surface-2/50 flex flex-col sm:flex-row gap-4 justify-between items-center">
                    <div className="relative w-full max-w-sm">
                      <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                      <input type="text" placeholder="Search faculty by name, department..." className="w-full pl-9 pr-4 py-2 text-sm rounded-lg border border-line focus:ring-2 focus:ring-blue-100 outline-none" />
                    </div>
                    <div className="flex gap-2 w-full sm:w-auto">
                      <button className="flex items-center justify-center gap-2 px-4 py-2 border border-line text-content font-bold text-sm rounded-lg hover:bg-surface-2 transition flex-1 sm:flex-none">
                        <Upload size={16} /> Bulk Import CSV
                      </button>
                      <button onClick={() => handleEditFaculty(null)} className="flex items-center justify-center gap-2 px-4 py-2 bg-brand text-white font-bold text-sm rounded-lg hover:bg-brand/90 transition flex-1 sm:flex-none">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left">
                      ...
                      <tbody className="text-sm divide-y divide-gray-100">
                        {facultyData.map(fac => (
                          <tr key={fac.id} className="hover:bg-surface-2/50 transition">
                            ...
                            <td className="p-4">
                              ...
                              <p className="text-xs text-muted">{fac.empId}</p>
                            </td>
                            <td className="p-4 font-medium text-content">{fac.dept}</td>
                            ...
                            <td className="p-4 text-muted text-xs">{fac.login}</td>
                            <td className="p-4 pr-6">
                              <div className="flex items-center gap-3">
                                <button onClick={() => handleEditFaculty(fac)} className="text-brand font-bold hover:underline text-xs">Edit</button>
                                <button onClick={() => setDeactivateModal({ open: true, id: fac.id, name: fac.name, action: fac.status === 'Active' ? 'Deactivate' : 'Activate' })} className="text-muted font-bold hover:text-red-600 transition text-xs">
                  ...
                  <div className="p-4 border-t border-gray-100 bg-surface-2 flex items-center justify-between text-xs text-muted font-medium">
                    <span>Showing 1-10 of 48</span>
                    <div className="flex items-center gap-2">
                      <button className="px-2 py-1 hover:bg-gray-200 rounded">Previous</button>
                      <button className="w-6 h-6 rounded bg-brand text-white font-bold flex items-center justify-center">1</button>
              ...
              {activeTab === 'Student Records' && (
                <div className="animate-fade-in">
                  <div className="p-4 border-b border-line bg-surface-2/50 flex flex-col gap-4">
                    <div className="flex flex-wrap gap-2 justify-between">
                      <div className="relative w-full max-w-sm">
                        <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                        <input type="text" placeholder="Search by name or roll number..." className="w-full pl-9 pr-4 py-2 text-sm rounded-lg border border-line focus:ring-2 focus:ring-blue-100 outline-none" />
                      </div>
                      <div className="flex gap-2">
                        <button className="px-4 py-2 border border-line text-content font-bold text-sm rounded-lg hover:bg-surface-2 transition">Export All</button>
                        <button className="px-4 py-2 border border-line text-content font-bold text-sm rounded-lg hover:bg-surface-2 transition">Bulk Import CSV</button>
                        <button className="px-4 py-2 bg-brand text-white font-bold text-sm rounded-lg hover:bg-brand/90 transition">Add Student</button>
                      </div>
                    </div>
                    <div className="flex gap-4">
                      <select className="text-sm border-line rounded-md bg-surface text-content py-1.5 px-3 border outline-none">
                      ...
                      <tbody className="text-sm divide-y divide-gray-100">
                        {students.map((st, i) => (
                          <tr key={i} className="hover:bg-surface-2/50 transition">
                            ...
                            <td className="p-4 text-content font-bold">{st.cgpa}</td>
                            <td className="p-4 text-xs text-muted">{st.status}</td>
                            <td className="p-4 pr-6 text-xs font-bold text-brand">
              ...
              {activeTab === 'Branch & Sections' && (
                <div className="p-6 animate-fade-in bg-surface-2 min-h-[500px]">
                  <div className="flex justify-end mb-4">
                    <button className="px-4 py-2 bg-brand text-white font-bold text-sm rounded-lg hover:bg-brand/90 transition">Add Branch</button>
                  </div>
                  <div className="space-y-4">
                    ...
                    {['CSE', 'IT', 'ECE'].map((branch) => {
                      ...
                      return (
                        <div key={branch} className="bg-surface rounded-xl shadow-card border border-line overflow-hidden">
                          <div 
                            className={`p-4 border-l-4 ${isCSE ? 'border-l-blue-600' : branch==='IT' ? 'border-l-teal-600' : 'border-l-purple-600'} flex items-center justify-between cursor-pointer hover:bg-surface-2 transition`}
                          ...
                          >
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-muted mt-1">Students: {students} · Sections: {sections} · Faculty: {facCount}</p>
                              </div>
                            </div>
                            <div className="flex items-center gap-4">
                              <span className="px-2 py-1 bg-green-100 text-green-700 text-[10px] font-bold uppercase rounded">Active</span>
                              <div className="flex gap-2">
                                <button onClick={(e) => e.stopPropagation()} className="px-3 py-1.5 border border-line text-gray-600 font-bold text-xs rounded hover:bg-surface-2">Edit Branch</button>
                                <button onClick={(e) => e.stopPropagation()} className="px-3 py-1.5 bg-gray-100 text-content font-bold text-xs rounded hover:bg-gray-200">View Details</button>
                              </div>
                            </div>
                          </div>

                          {expandedBranch === branch && (
                            <div className="p-6 border-t border-gray-100 bg-surface-2/50">
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                ...
                                <div className="bg-surface p-4 rounded-xl border border-line">
                                  ...
                                  <div className="space-y-3 mb-4">
                                    <div className="text-sm"><p className="font-bold text-content">Section A</p><p className="text-xs text-muted">80 students · CT: Dr. Anita Sharma</p></div>
                                    <div className="text-sm"><p className="font-bold text-content">Section B</p><p className="text-xs text-muted">80 students · CT: Prof. Kavya Nair</p></div>
                                    <div className="text-sm"><p className="font-bold text-content">Section C</p><p className="text-xs text-muted">80 students · CT: Dr. Ravi Sharma</p></div>
                                    <div className="text-sm"><p className="font-bold text-content">Section D</p><p className="text-xs text-muted">80 students · CT: Prof. Priya Kapoor</p></div>
                                  </div>
                                  <button className="w-full py-1.5 bg-brand-soft text-brand font-bold text-xs rounded border border-brand/20 hover:bg-brand/10 transition">Add Section</button>
                                </div>
                                ...
                                <div className="bg-surface p-4 rounded-xl border border-line">
                                  ...
                                  <div className="space-y-3 mb-4">
                                    <div className="text-sm"><p className="font-bold text-content">Section A</p><p className="text-xs text-muted">61 students · CT: Prof. Priya Kapoor</p></div>
                                    <div className="text-sm"><p className="font-bold text-content">Section B</p><p className="text-xs text-muted">60 students · CT: Dr. Suresh Iyer</p></div>
                                    <div className="text-sm"><p className="font-bold text-content">Section C</p><p className="text-xs text-muted">59 students · CT: Prof. Kavya Nair</p></div>
                                  </div>
                                  <button className="w-full py-1.5 bg-brand-soft text-brand font-bold text-xs rounded border border-brand/20 hover:bg-brand/10 transition">Add Section</button>
                                </div>
                                ...
                                <div className="bg-surface p-4 rounded-xl border border-line">
                                  ...
                                  <div className="space-y-3 mb-4">
                                    <div className="text-sm"><p className="font-bold text-content">Section A</p><p className="text-xs text-muted">148 students (combined) · CT: Dr. Anita Sharma</p></div>
                                  </div>
                                  <button className="w-full py-1.5 bg-brand-soft text-brand font-bold text-xs rounded border border-brand/20 hover:bg-brand/10 transition">Add Section</button>
                                </div>
                                ...
                                <div className="bg-surface p-4 rounded-xl border border-line">
                                  ...
                                  <div className="space-y-3 mb-4">
                                    <div className="text-sm"><p className="font-bold text-content">Section A</p><p className="text-xs text-muted">132 students (combined) · CT: Dr. Suresh Iyer</p></div>
                                    <div className="text-sm"><p className="font-bold text-content">Section B</p><p className="text-xs text-muted">128 students (combined) · CT: Dr. Anita Sharma</p></div>
                                  </div>
                                  <button className="w-full py-1.5 bg-brand-soft text-brand font-bold text-xs rounded border border-brand/20 hover:bg-brand/10 transition">Add Section</button>
                                </div>
              ...
              {activeTab === 'Subject Mapping' && (
                <div className="animate-fade-in">
                  <div className="p-4 border-b border-line bg-surface-2/50 flex justify-between items-center">
                    <div className="flex gap-4">
                      <select className="text-sm border-line rounded-md bg-surface text-content py-1.5 px-3 border outline-none">
                    ...
                    <button className="px-4 py-2 bg-brand text-white font-bold text-sm rounded-lg hover:bg-brand/90 transition">Add Subject</button>
                  </div>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {subjectsData.map((sub, i) => (
                        <tr key={i} className="hover:bg-surface-2/50">
                          ...
                          <td className="p-4 text-muted">{sub.cred} cr · {sub.co} COs</td>
                          <td className="p-4 pr-6 text-right font-bold text-brand text-xs">
              ...
              {activeTab === 'System Logs' && (
                <div className="animate-fade-in">
                  <div className="p-4 border-b border-line bg-surface-2/50 flex flex-wrap gap-4 justify-between">
                    <div className="flex gap-4">
                      <select className="text-sm border-line rounded-md bg-surface text-content py-1.5 px-3 border outline-none">
                    ...
                    <div className="relative max-w-xs w-full">
                      <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                      <input type="text" placeholder="Search logs..." className="w-full pl-9 pr-4 py-1.5 text-sm rounded-lg border border-line focus:ring-2 focus:ring-blue-100 outline-none" />
                    </div>
                  </div>
                  <table className="w-full text-left text-sm">
                    <thead>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {systemLogs.map((log, i) => (
                        <tr key={i} className={`hover:bg-surface-2/50 ${log.status === 'Failed' ? 'bg-red-50/30' : ''}`}>
                          <td className="p-4 pl-6 text-muted text-xs">{log.time}</td>
                          <td className="p-4 font-bold text-navy">{log.user}</td>
                          <td className="p-4 text-gray-600">{log.type}</td>
                          <td className="p-4 text-content">{log.desc}</td>
      ...
      {editPanelOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-[450px] bg-surface h-full shadow-2xl flex flex-col animate-slide-in-right">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-surface-2/50">
              <h2 className="text-xl font-bold text-navy">{editingFaculty ? `Edit Faculty — ${editingFaculty.name}` : 'Add New Faculty'}</h2>
              <button onClick={() => setEditPanelOpen(false)} className="text-gray-400 hover:text-content"><X size={20}/></button>
            </div>
            
            <div className="p-6 flex-1 overflow-y-auto space-y-5">
              <div>
                <label className="block text-xs font-bold text-content mb-1.5">Full Name</label>
                <input type="text" defaultValue={editingFaculty?.name || ''} className="w-full px-3 py-2 border border-line rounded-lg text-sm focus:ring-2 focus:ring-blue-100 outline-none" />
              </div>
              <div>
                <label className="block text-xs font-bold text-content mb-1.5">Employee ID</label>
                <input type="text" defaultValue={editingFaculty?.empId || ''} disabled={!!editingFaculty} className="w-full px-3 py-2 border border-line bg-surface-2 text-muted rounded-lg text-sm" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-content mb-1.5">Department</label>
                  <select defaultValue={editingFaculty?.dept || 'CSE'} className="w-full px-3 py-2 border border-line rounded-lg text-sm bg-surface outline-none">
                    <option>CSE</option><option>IT</option><option>ECE</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-bold text-content mb-1.5">Experience</label>
                  <input type="text" defaultValue="6 years" className="w-full px-3 py-2 border border-line rounded-lg text-sm outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-bold text-content mb-1.5">Subjects</label>
                <div className="flex flex-wrap gap-2 p-2 border border-line rounded-lg mb-2">
                  {(editingFaculty?.subjects || []).map(s => (
                    <span key={s} className="px-2 py-1 bg-gray-100 border border-line text-xs font-bold text-navy rounded flex items-center gap-1">
                ...
                <select className="w-full px-3 py-2 border border-line rounded-lg text-sm bg-surface text-muted outline-none">
                  <option>Add Subject...</option>
                </select>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-content mb-1.5">Email</label>
                  <input type="email" placeholder="email@college.edu" className="w-full px-3 py-2 border border-line rounded-lg text-sm outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-content mb-1.5">Phone</label>
                  <input type="text" placeholder="+91 9876543210" className="w-full px-3 py-2 border border-line rounded-lg text-sm outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-bold text-content mb-1.5">Qualification</label>
                <select className="w-full px-3 py-2 border border-line rounded-lg text-sm bg-surface outline-none">
                  <option>PhD</option><option>MTech</option><option>BTech</option><option>Other</option>
                </select>
              </div>
              <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                <label className="text-sm font-bold text-content">Account Status</label>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-muted">Inactive</span>
                  <div className={`w-10 h-5 rounded-full relative cursor-pointer ${(!editingFaculty || editingFaculty.status === 'Active') ? 'bg-green-500' : 'bg-gray-300'}`}>
                    <div className={`w-3 h-3 bg-surface rounded-full absolute top-1 transition-all ${(!editingFaculty || editingFaculty.status === 'Active') ? 'left-6' : 'left-1'}`} />
                  </div>
            ...
            <div className="p-4 border-t border-gray-100 bg-surface-2 flex gap-3">
              <button onClick={() => { showToast('Faculty details saved'); setEditPanelOpen(false); }} className="flex-1 py-2 bg-brand text-white font-bold text-sm rounded-lg hover:bg-brand/90 transition">
                Save Changes
              </button>
              <button onClick={() => setEditPanelOpen(false)} className="flex-1 py-2 bg-surface border border-line text-content font-bold text-sm rounded-lg hover:bg-surface-2 transition">
                Cancel
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="bg-surface rounded-2xl shadow-2xl w-[400px] overflow-hidden relative z-10 animate-fade-in p-6 text-center">
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeactivateModal({ open: false, id: null, name: '' })} className="flex-1 py-2.5 bg-gray-100 text-content font-bold rounded-lg hover:bg-gray-200 transition">Cancel</button>
