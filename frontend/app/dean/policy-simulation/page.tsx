...
const BRANCH_SENSITIVITY: Record<string, number> = { CSE: 1.1 };

const IMPACT_MULTIPLIER: Record<string, number> = { Low: 0.5, Medium: 1.0, High: 1.6 };
...
const POLICY_RULES: Record<string, (config: PolicyConfig) => Record<string, number>> = {
  "Add Course": (config) => {
...
function runSimulation(config: PolicyConfig) {
...
// ─── ANIMATED COUNTER ─────────────────────────────────────────────────────────
function AnimatedNumber({ value, decimals = 1, duration = 900 }: { value: number | string; decimals?: number; duration?: number }) {
...
export default function PolicySimulation() {
  ...
  const [savedScenarios, setSaved] = useState<{ label: string; impactLevel: string; targetBranch: string; result: ReturnType<typeof runSimulation>; id: number }[]>([]);
  const [compareMode, setCompareMode] = useState(false);
  const [compareB, setCompareB] = useState<ReturnType<typeof runSimulation>>(null);
  ...
  const handleCompareLoad = (s: { result: ReturnType<typeof runSimulation> }) => {
