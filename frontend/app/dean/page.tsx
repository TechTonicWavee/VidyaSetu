...
function TrendPill({ trend, label }: { trend?: string | null; label?: string | null }) {
  ...
  } as Record<string, { Icon: typeof ArrowUp; cls: string }>)[trend]
  if (!cfg) return null
  return (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
        ...
        <div className="card xl:col-span-5 animate-fade-in" style={{ animationDelay: '0.4s' }}>
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => router.push('/dean/meetings')} className="text-xs font-bold text-teal-600 hover:underline">View All</button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {upcomingMeetings.length > 0 ? (
              ...
              <div className="col-span-3 py-8 text-center bg-gray-50/50 rounded-xl border border-dashed border-gray-200">
                ...
                <button onClick={() => router.push('/dean/agent')} className="mt-2 text-xs font-bold text-teal-600 hover:underline">+ Schedule with AI Agent</button>
              </div>
            )}
          </div>
        </div>
