...
import { 
  ...
  Download, RefreshCw, MoreHorizontal, User, Clock
} from 'lucide-react'
...
const navLinks = [
  ...
  { id: 'attendance', label: 'Attendance (Vidya)',   icon: CheckCircle,badge: null, active: true,  path: '/dashboard/faculty/attendance' },
]
        ...
        <nav className="flex-1 p-3 overflow-y-auto">
          {navLinks.map(link => (
            <button
              ...
              className={`nav-link w-full text-left mb-0.5 ${link.active ? 'bg-teal-50 text-teal-700 font-semibold' : ''}`}
            >
              <link.icon size={17} />
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-gray-100 px-6 py-3 flex items-center gap-4 flex-shrink-0 shadow-sm">
          ...
          <div className="flex-1 max-w-md relative">
            ...
            <input type="text" placeholder="Search students..." className="w-full pl-9 pr-4 py-2 text-sm rounded-lg border border-gray-200 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-teal-100 focus:border-teal-300 transition" />
          </div>
          <div className="flex-1" />
          <button onClick={handleSync} className={`p-2 rounded-lg hover:bg-gray-100 transition text-teal-600 flex items-center gap-2 text-sm font-bold ${syncing ? 'animate-pulse' : ''}`}>
          ...
          <div className="max-w-6xl mx-auto space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              ...
              <div className="flex items-center gap-3 w-full md:w-auto">
                ...
                <button className="flex-1 md:flex-none px-4 py-2 bg-teal-600 text-white font-bold text-sm rounded-xl hover:bg-teal-700 transition shadow-sm flex items-center justify-center gap-2">
                      ...
                      <tr key={s.id} className="hover:bg-gray-50/30 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-col items-center">
                            <span className={`text-sm font-bold ${s.attendance < 75 ? 'text-red-600' : 'text-teal-600'}`}>
                              {s.attendance}%
                            </span>
                            <div className="w-24 bg-gray-100 h-1 rounded-full mt-1.5 overflow-hidden">
                              <div className={`h-full rounded-full ${s.attendance < 75 ? 'bg-red-500' : 'bg-teal-500'}`} style={{ width: `${s.attendance}%` }} />
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                            ...
                            s.status === 'Good' ? 'bg-teal-100 text-teal-700' :
